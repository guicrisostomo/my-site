...
export default function Home() {
  ...
  const getData = async () => {
    ...
    const octokit = new Octokit({
      auth: process.env.REACT_APP_VERCEL_GITHUB_TOKEN,
    });
  ...
  function reverseDateEN(str) {
    return (
      str.split("-")[1] + "/" + str.split("-")[2] + "/" + str.split("-")[0]
    );
  }
  ...
  function subHour(time) {
    ...
    ).toLocaleTimeString(i18n.languages, options);
    return date;
  }
  ...
  return (
    <>
      ...
      <SectionAbout>
        ...
        <Subtitle>
          Tempo total: {codeTime ? (codeTime.timeTotal / 3600).toFixed(0) : ""}{" "}
          horas e{" "}
          {codeTime
            ? (codeTime.timeTotal / 60).toFixed(0) -
              (codeTime.timeTotal / 3600).toString().split(".")[0] * 60
            : ""}{" "}
          minutos
          <br />
          Tempo codando:{" "}
          {codeTime
            ? (codeTime.timeCoding / 3600).toString().split(".")[0]
            : ""}{" "}
          horas e{" "}
          {codeTime
            ? (codeTime.timeCoding / 60).toFixed(0) -
              (codeTime.timeCoding / 3600).toString().split(".")[0] * 60
            : ""}{" "}
          minutos
          <br />
          Tempo navegando:{" "}
          {codeTime
            ? (codeTime.timeBrowsing / 3600).toString().split(".")[0]
            : ""}{" "}
          horas e{" "}
          {codeTime
            ? (codeTime.timeBrowsing / 60).toFixed(0) -
              (codeTime.timeBrowsing / 3600).toString().split(".")[0] * 60
            : ""}{" "}
          minutos
          <br />
          <br />
      ...
      <SectionEvents>
        <Title>{t("home.event.title")}</Title>

        <Subtitle>{t("home.event.text")}</Subtitle>
              ...
              <ItemEventInfo key={item.id}>
                <ItemEventInfoTexts>
                  <ItemEventInfoName>
                    {item.repo.name
                      .toString()
                      .replace("guicrisostomo/", "")
                      .replaceAll("-", " ")}
                  </ItemEventInfoName>

                  <ItemEventInfoText>
                    {t("home.event.typeEvent") +
                      separateUpperLetters(item.type.toString().toLowerCase())}
